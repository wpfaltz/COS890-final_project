...
Usage:
    python scripts/run_benchmark.py --customers 25 50 --time-limit 120 --instances c101 r101 rc101
"""
...
MASTER_KEY_COLS = ["instance", "customers", "method"]


def upsert_master(row: dict, master_path: Path) -> None:
    """Append a single result row to the master table, replacing any existing row
    with the same (instance, customers, method) key so reruns overwrite stale results."""
    new_df = pd.DataFrame([row])
    if master_path.exists():
        df = pd.read_csv(master_path)
        keep = ~(
            (df["instance"] == row["instance"])
            & (df["customers"] == row["customers"])
            & (df["method"] == row["method"])
        )
        df = pd.concat([df[keep], new_df], ignore_index=True)
    else:
        df = new_df
    df = df.sort_values(MASTER_KEY_COLS).reset_index(drop=True)
    master_path.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(master_path, index=False)


def run_one(name: str, num_customers: int, time_limit: float, include_reference: bool = True) -> Iterator[dict]:
    ...
    yield {
        ...
        "vehicles": lr.num_vehicles, "root_lp_bound": lr.root_lp_bound,
...
def main() -> None:
    ...
    parser.add_argument("--customers", type=int, nargs="+", default=[25])
    ...
    parser.add_argument("--master-out", default=None,
                         help="path to the single master table CSV, updated incrementally after every "
                              "instance/method run (default: results/benchmark_master.csv)")
    args = parser.parse_args()

    out_dir = Path(args.out_dir) if args.out_dir else RESULTS_DIR
    out_dir.mkdir(parents=True, exist_ok=True)
    master_path = Path(args.master_out) if args.master_out else RESULTS_DIR / "benchmark_master.csv"

    for num_customers in args.customers:
        all_rows = []
        for name in args.instances:
            print(f"solving {name} ({num_customers} customers)...")
            rows = []
            for row in run_one(name, num_customers, args.time_limit, include_reference=not args.no_reference):
                rows.append(row)
                upsert_master(row, master_path)
                print(f"  {row['method']}: done, merged into {master_path}")
            all_rows.extend(rows)

            df = pd.DataFrame(rows)
            out_path = out_dir / f"benchmark_{name}_{num_customers}.csv"
            # df.to_csv(out_path, index=False)
            print(df.to_string(index=False))
            print(f"saved to {out_path}\n")

        combined = pd.DataFrame(all_rows)
        combined_path = out_dir / f"benchmark_{num_customers}_all.csv"
        # combined.to_csv(combined_path, index=False)
        print(f"saved combined results to {combined_path}")

    print(f"master table up to date at {master_path}")
