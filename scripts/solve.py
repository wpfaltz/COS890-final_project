...
def main() -> None:
    ...
    parser.add_argument("--method", choices=["bb", "bc", "lr"], default="bc",
    ...
    if args.method in ("bb", "bc"):
        solve_fn = solve_branch_and_bound if args.method == "bb" else solve_branch_and_cut
        ...
        print(f"status={result.status} vehicles={result.num_vehicles} distance={result.total_distance:.2f} "
              f"lower_bound={result.lower_bound:.2f} gap={result.mip_gap:.4%} runtime={result.runtime:.2f}s")
        ...
        print(f"lower_bound={lr.best_lower_bound:.2f} upper_bound={lr.upper_bound:.2f} gap={gap:.4%} "
