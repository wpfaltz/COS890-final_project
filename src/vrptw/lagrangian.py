...
def _solve_espprc(
    ...
    # label = (time, node, cost, load, visited_mask, path)
    counter = itertools.count()
    start = (0.0, next(counter), depot, 0.0, 0.0, 0, (depot,))
    heap = [start]
    ...
    while heap:
        if deadline is not None and explored % 2000 == 0 and _time.time() > deadline:
            exact = False
            break
        arrival, _, node, cost, load, visited, path = heapq.heappop(heap)
        explored += 1
        ...
        for j in customers:
            ...
            heapq.heappush(
                heap,
                (new_arrival, next(counter), j, new_cost, new_load, new_visited, path + (j,)),
            )
        ...
        if len(path) > 1:  # allow returning to depot only after visiting at least one customer
            ...
            if new_arrival <= inst.due_date[depot] + 1e-9:
                ...
                heapq.heappush(
                    heap,
                    (new_arrival, next(counter), depot, new_cost, load, visited, path + (depot,)),
                )

    return SubproblemResult(cost=best_cost, path=best_path, exact=exact)
...
def lagrangian_relaxation(
    ...
    while max_iterations is None or it < max_iterations:
        ...
        # No upper bound is tracked, so we can't use the classical Polyak step
        # alpha*(UB-LB)/||g||^2 -- fall back to a diminishing step alpha/||g||^2,
        # with alpha itself halved on stall (below) to keep it square-summable.
        step = alpha / norm_sq
        for c in customers:
        ...
        if verbose:
            print(f"iter {it}: lb={lb:.2f} best_lb={best_lb:.2f} alpha={alpha:.4f} step={step:.4f}")
